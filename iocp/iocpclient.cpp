...
volatile LONG gTotalOutStandingOps = 0;
...
void PrintStatistics()
{
    ...
    printf("Total Connections  : %lu\n", gTotalConnections);
    printf("Connections Refused: %lu\n", gConnectionRefused);
    printf("Current Total OutStandingOps: %lu\n", gTotalOutStandingOps);
...
int PostRecv(SOCKET_OBJ *sock, BUFFER_OBJ *recvobj)
{
    ...
    InterlockedIncrement(&gTotalOutStandingOps);
...
int PostSend(SOCKET_OBJ *sock, BUFFER_OBJ *sendobj)
{
    ...
    if (rc == SOCKET_ERROR)
    {
        ...
        if (err != WSA_IO_PENDING)
        {
			if (err == WSAENOBUFS)
			{
				fprintf(stderr, "PostSend: WSASend* failed: %d , WSAENOBUFS. \n", WSAGetLastError());
...
int HandleIo(SOCKET_OBJ *sock, BUFFER_OBJ *buf, DWORD BytesTransfered, DWORD error)
{
    ...
    else
    {
        ...
        else if (buf->operation == OP_READ)
        {
			...
			//if ((i % 5000) == 0)
			//{
			//	fprintf(stderr, "Received!!!! id=%I64d\n", buf->id);
			//}
    ...
    //
	InterlockedDecrement(&gTotalOutStandingOps);
