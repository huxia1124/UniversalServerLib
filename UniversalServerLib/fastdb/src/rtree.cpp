...
#define F_INFINITY (area_t)1000000000*1000000000
...
oid_t dbRtreePage::add_branch(dbDatabase* db, oid_t pageId, branch const& br)  
{
    ...
    int i, j, seed[2] = {0,0};
    area_t rect_area[card+1], waste, worst_waste = -F_INFINITY;
...
oid_t dbRtreePage::insert(dbDatabase* db, rectangle const& r, oid_t pageId, oid_t recordId, int level)
{
    ...
    if (--level != 0) { 
        // not leaf page
        int i, mini = 0;
        area_t min_incr = F_INFINITY;
        area_t best_area = F_INFINITY;
        for (i = 0; i < n; i++) { 
